...
import KpisModal from "@/modal/KpisModal"; // Importa o modal de KPIs

const Sidebar = forwardRef(({ collapsed }, ref) => {
  const [openMenus, setOpenMenus] = useState({});
  const [activeModal, setActiveModal] = useState(null);

  // Supondo que a role esteja salva no localStorage
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex w-full flex-col gap-y-4 overflow-y-auto overflow-x-hidden p-3 [scrollbar-width:_thin]">
          {navbarLinks.map((section) => {
            // Filtra os links com base na role do usuário
            ...
            return (
              <nav key={section.title} className={cn("sidebar-group", collapsed && "md:items-center")}>
                <p
                  className={cn(
                    "sidebar-group-title text-gray-600 dark:text-gray-300 font-semibold text-sm",
                    collapsed && "md:w-[45px]"
                  )}
                >
                  {section.title}
                  ...
                  link.dropdown ? (
                          ...
                          {link.children.map((child) =>
                            child.modal ? (
                              ...
                              <NavLink
                                key={child.label}
                                to={child.path}
                                className="flex items-center gap-2 w-full p-2 rounded hover:bg-gray-200 dark:text-white dark:hover:bg-gray-600 transition-all duration-500 ease-in-out"
                              >
                                {child.icon && <child.icon className="h-5 w-5" />}
                                <span className="text-gray-700 dark:text-gray-300 font-medium">
                                  {child.label}
                    ...
                    <NavLink
                      ...
                      className={cn(
                        "sidebar-item",
                        collapsed && "md:w-[45px]",
                        "hover:bg-gray-200 dark:text-white dark:hover:bg-gray-600 transition-all duration-500 ease-in-out"
                      )}
                    >
                      {link.icon && <link.icon className="mr-2 h-5 w-5" />}
                      {!collapsed && (
      ...
      {activeModal === "KPIs -TESTE" && <KpisModal isOpen={true} onClose={closeModal} />}
    </>
  );
});
