...
const MaintenanceModal = ({ isOpen, onClose }) => {
  ...
  const fetchMaintenanceData = async () => {
    setLoading(true);
    try {
      const querySnapshot = await getDocs(collection(db, "Pendente")); // Busca apenas os documentos "Pendentes"
      const data = querySnapshot.docs.map((doc) => doc.data()).filter(event => event.type === "manutencao");
  ...
  const formatMessage = () => {
    if (maintenanceData.length === 0) {
      return "Nenhuma manutenção pendente registrada no momento.";
    }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-96">
        <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">Manutenções Pendentes</h2>
        {loading ? (
